...
const MyReviewsScreen = ({ navigation }) => {
  ...
  const [editTarget, setEditTarget] = useState(null);   // review object to edit
  const [deleteTarget, setDeleteTarget] = useState(null); // review object to delete
  const [editModalVisible, setEditModalVisible] = useState(false);

  useEffect(() => {
    dispatch(loadMyReviews(accessToken));
  }, []);
  ...
  const renderItem = ({ item }) => {
    ...
    const badgeBg = CATEGORY_COLORS[category] || '#3a3a3a';
    const badgeText = CATEGORY_TEXT_COLORS[category] || '#ffffff';
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardBody}>
          ...
          <Text style={styles.dateText}>
            ...
            {item.verifiedPurchase && (
              <Text style={styles.verifiedInline}> · ✓ Verified Purchase</Text>
            )}
          </Text>
        </View>
        ...
        <View style={styles.cardActions}>
          <TouchableOpacity
            ...
            onPress={() => { setEditTarget(item); setEditModalVisible(true); }}
          >
...
const styles = StyleSheet.create({
  ...
  verifiedInline: { color: '#38b6ff' },
  cardActions: { flexDirection: 'column', gap: 8, padding: 10, justifyContent: 'center' },
  editBtn: {
    backgroundColor: '#ffde59', width: 32, height: 32, borderRadius: 8, alignItems: 'center', justifyContent: 'center',
  },
  deleteBtn: {
    backgroundColor: '#ff3131', width: 32, height: 32, borderRadius: 8, alignItems: 'center', justifyContent: 'center',
  },
  ...
  modalTitle: {
    fontFamily: 'Oswald_700Bold', fontSize: 22, fontStyle: 'italic', color: '#ffffff',
  ...
  cancelBtn: {
    flex: 1, backgroundColor: '#3a3a3a', borderRadius: 6, height: 48, alignItems: 'center', justifyContent: 'center',
  },
  cancelText: { fontFamily: 'Montserrat_700Bold', color: '#ffffff', fontWeight: '700', fontSize: 14 },
  confirmBtn: {
    flex: 1, backgroundColor: '#ff3131', borderRadius: 6, height: 48, alignItems: 'center', justifyContent: 'center',
  },
